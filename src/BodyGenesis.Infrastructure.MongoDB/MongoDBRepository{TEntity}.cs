...
namespace BodyGenesis.Infrastructure.MongoDB
{
        ...
        where TEntity : EntityBase
    {
        ...
        public async Task<IReadOnlyCollection<TEntity>> Query(Expression<Func<TEntity, bool>> filterExpression, int skip = 0, int take = int.MaxValue, Expression<Func<TEntity, object>> sortExpression = null, QuerySortDirection sortDirection = QuerySortDirection.Ascending)
        {
            SortDefinition<TEntity> sort = null;

            if (sortExpression is not null)
            {
                if (sortDirection == QuerySortDirection.Ascending)
                {
                    sort = Builders<TEntity>.Sort.Ascending(sortExpression);
                }

                else
                {
                    sort = Builders<TEntity>.Sort.Descending(sortExpression);
                }
            }

            return await _collection.Find(filterExpression).Sort(sort).Skip(skip).Limit(take).ToListAsync();
        }
